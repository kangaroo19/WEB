...
function paintToDo(newToDoObj){
    ...
    li.id=newToDoObj.id; //html li 의 id를 객체로 전달받은 값(newToDoObjObj)의 id(date.now)로 지정
...
function handleToDoSubmit(event){
    ...
    toDos.push(newToDoObj);//객체 newToDoObj를 배열toDos에 추가
    paintToDo(newToDoObj);
    saveToDos();
}
toDoForm.addEventListener("submit",handleToDoSubmit);

function sayHello(item){    //?item은 배열의 요소(타이핑한 값)인듯? 매개변수가 꼭 item이 아니어도 됨
